...
contract('VestingVault', function ([ owner, other ]) {
  ...
  beforeEach(async function () {
    ...
    this.vault = await VestingVault.new(this.hyp.address)
    await this.hyp.approve(this.vault.address, 1000);
  });
  ...
  it('can claim vested tokens', async function() {
    ...
    expect((await this.hyp.balanceOf(other)).toString()).to.equal("300");
  });
  ...
  it('does not release tokens before cliff is up', async function() {
    ...
    await time.increase(time.duration.days(1));
    await this.vault.claimVestedTokens({from: other})
    expect((await this.hyp.balanceOf(other)).toString()).to.equal("200");

    await time.increase(time.duration.days(1));
    await this.vault.claimVestedTokens({from: other})
    expect((await this.hyp.balanceOf(other)).toString()).to.equal("400");

    await time.increase(time.duration.days(1));
    await this.vault.claimVestedTokens({from: other})
    expect((await this.hyp.balanceOf(other)).toString()).to.equal("600");

    await time.increase(time.duration.days(1));
    await this.vault.claimVestedTokens({from: other})
    expect((await this.hyp.balanceOf(other)).toString()).to.equal("800");
  ...
  it('releases balance at end if uneven vest', async function() {
    ...
    await this.vault.claimVestedTokens({from: other})
    expect((await this.hyp.balanceOf(other)).toString()).to.equal("333");

    await time.increase(time.duration.days(1));
    await this.vault.claimVestedTokens({from: other})
    expect((await this.hyp.balanceOf(other)).toString()).to.equal("666");
  ...
  it('owner can revoke token grant', async function() {
    await this.vault.addTokenGrant(other, 1000, 3, 7);
    expect((await this.hyp.balanceOf(this.vault.address)).toString()).to.equal("1000");

    await time.increase(time.duration.days(8));
    await this.vault.revokeTokenGrant(other);

    expect((await this.hyp.balanceOf(owner)).toString()).to.equal("334");
    expect((await this.hyp.balanceOf(other)).toString()).to.equal("666");
    expect((await this.hyp.balanceOf(this.vault.address)).toString()).to.equal("0");
  });
});
